...
test('Test Case 3:Verify that Password reset email should be sent to email address', async ({ page }) => {
    await page.goto(process.env.QUICK_TEST_BASE_URL!)
    await page.getByRole('link', { name: 'Forgot password?' }).click()
    await page.waitForURL(process.env.QUICK_TEST_FORGOT_PASSWORD!)
...
test('Test Case 4:Verify that after clicking on reset password link user should be redirected to "Setup your new password" screen', async ({ page }) => {
    await page.goto(process.env.QUICK_TEST_BASE_URL!)
    await page.getByRole('link', { name: 'Forgot password?' }).click()
    await page.waitForURL(process.env.QUICK_TEST_FORGOT_PASSWORD!)
    await page.fill('#email',process.env.QUICK_USERNAME!)
...
test('Test Case 5:Verify that user can change password', async ({ page }) => {
    await page.goto(process.env.QUICK_TEST_BASE_URL!)
    await page.getByRole('link', { name: 'Forgot password?' }).click()
    await page.waitForURL(process.env.QUICK_TEST_FORGOT_PASSWORD!)
    await page.fill('#email',process.env.QUICK_USERNAME!)
...
test('Test Case 7:Verify when user clicks on quick test logo on set up new password screen it should be redirected to sign in page', async ({ page }) => {
   await page.goto(process.env.QUICK_TEST_BASE_URL!)
    await page.getByRole('link', { name: 'Forgot password?' }).click()
    await page.waitForURL(process.env.QUICK_TEST_FORGOT_PASSWORD!)
    await page.fill('#email',process.env.QUICK_USERNAME!)
...
test('Test Case 12: Verify that the forgot password link can be used only once and shows an error on reuse.', async ({ page }) => {
    await page.goto(process.env.QUICK_TEST_BASE_URL!);
    await page.getByRole('link', { name: 'Forgot password?' }).click();
    await page.waitForURL(process.env.QUICK_TEST_FORGOT_PASSWORD!);
    await page.fill('#email', process.env.QUICK_USERNAME!);
...
test('Test Case 13: Verify that user is able to login with the new password set ', async ({ page }) => {
     await page.goto(process.env.QUICK_TEST_BASE_URL!)
    await page.getByRole('link', { name: 'Forgot password?' }).click()
    await page.waitForURL(process.env.QUICK_TEST_FORGOT_PASSWORD!)
    await page.fill('#email',process.env.QUICK_USERNAME!)
