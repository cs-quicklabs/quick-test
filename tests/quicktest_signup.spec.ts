...
test('Test Case 3:Verify after entering all required fields and clicking on sign up button new account should be created', async ({ page }) => {
    ...
    await page.fill('#password',process.env.QUICK_PASSWORD!+'3dex8vP@1')
    await page.fill('#cnfpassword',process.env.QUICK_PASSWORD!+'3dex8vP@1')
...
test.skip('Test Case 6:Verify that after clicking on either i agree to terms of use or  privacy policy link button,user should be redirected to respective page', async ({ page }) => {
    await page.goto(process.env.QUICK_TEST_BASE_URL!)
    await page.getByRole('link', { name: 'Sign up' }).click()
